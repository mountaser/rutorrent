...
XMLRPC_SIZE_LIMIT=${XMLRPC_SIZE_LIMIT:-16M}
...
RU_CACHED_PLUGIN_LOADING=${RU_CACHED_PLUGIN_LOADING:-true}
...
if [ -f "${CONFIG_PATH}/rtorrent/.rtorrent.rc" ]; then
  ...
  sed -i -E 's/\(\(close_low_diskspace,([0-9]+[KMGT])\)$/\(\(close_low_diskspace,\1\)\)/g' "${CONFIG_PATH}/rtorrent/.rtorrent.rc"
...
mkdir -p ${CONFIG_PATH}/rtorrent
touch ${CONFIG_PATH}/rtorrent/.rtstate.rc
...
# Patch httprpc plugin for rtorrent 0.16.x compatibility
if [ -f /var/www/rutorrent/plugins/httprpc/action.php ]; then
  echo "    ${norm}[${blue}+${norm}] Patching ruTorrent ${green}httprpc${norm} plugin for rtorrent 0.16.x..."
  sed -i 's/"main"/""/g' /var/www/rutorrent/plugins/httprpc/action.php
fi
