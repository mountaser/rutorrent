...
* **Graceful Tracker Shutdown:** On container termination, rTorrent announces `event=stopped` to all trackers before exiting, preventing "seeding from multiple locations" flags on private trackers. The shutdown supervisor caps wait time to 3s to guarantee clean container stops via `docker stop`.
